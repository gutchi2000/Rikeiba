...
st.table(top6)
tag_colors = ['#e31a1c','#1f78b4','#33a02c','#ff7f00','#6a3d9a','#6a3d9a']
import seaborn as sns
fig1, ax1 = plt.subplots(figsize=(8,5))
sns.barplot(x='平均偏差値', y='馬名', data=top6, palette=tag_colors, ax=ax1)
st.pyplot(fig1)
...
with st.expander('券種別買い目候補と予算配分'):
    ...
    alloc = {}; base = total_budget // len(types)
    for t in types[:-1]: alloc[t] = (base // 100) * 100
    alloc[types[-1]] = ((total_budget - sum(alloc.values())) // 100) * 100
    alloc_rows = []
    for t, amt in alloc.items():
        if t == '単勝': alloc_rows.append({'券種':'単勝','組合せ':axis,'金額':int((amt * 1/4) // 100 * 100)})
        elif t == '複勝': alloc_rows.append({'券種':'複勝','組合せ':axis,'金額':int((amt * 3/4) // 100 * 100)})
        elif t == '馬連': per = ((amt // 5) // 100 * 100)
            for c in umaren: alloc_rows.append({'券種':'馬連','組合せ':c,'金額':per})
        elif t == 'ワイド': per = ((amt // 5) // 100 * 100)
            for c in wide: alloc_rows.append({'券種':'ワイド','組合せ':c,'金額':per})
        elif t == '三連複': per = ((amt // len(sanrenpuku)) // 100 * 100)
            for c in sanrenpuku: alloc_rows.append({'券種':'三連複','組合せ':c,'金額':per})
        elif t == '三連単': per = ((amt // len(sanrentan)) // 100 * 100)
            for c in sanrentan: alloc_rows.append({'券種':'三連単','組合せ':c,'金額':per})
