...
horses = edited[['枠', '番', '馬名', '性別', '年齢', '脚質']].copy()
...
points = alt.Chart(df_agg).mark_circle(size=100).encode(
    x='RankZ:Q', y='Stability:Q', tooltip=['馬名', 'AvgZ', 'Stdev']
)
...
st.subheader("■ 資金配分")
st.write(
    f"合計予算：{total_budget:,}円 ｜ 単勝：{pur1:,}円 ｜ 複勝：{pur2:,}円 ｜ 残り：{rem:,}円"
)
...
if scenario == '通常':
    ...
    n_lines = len(others_names)
    ideal = rem / n_lines
    base = int(np.floor(ideal / 100) * 100)
    total_assigned = base * n_lines
    leftover = rem - total_assigned
    for idx, (nm, mk) in enumerate(zip(others_names, others_symbols)):
        amt = base + (leftover if idx == 0 else 0)
    ...
    n_w = len(others_names)
    n_t3 = len(list(combinations(others_names, 2)))
    ...
    for pair in combinations(others_names, 2):
        bets.append({'券種': '三連複', '印': '◎-〇▲☆△△', '馬': h1, '相手': '／'.join(pair), '金額': base})
    for idx, (s, t) in enumerate(combo3):
        amt = base + (leftover if idx == 0 else 0)
