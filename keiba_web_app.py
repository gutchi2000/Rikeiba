...
# 安全なリネーム（列が存在しない場合 KeyError を防ぐ）
...
df = df.merge(
    edited[['馬名','脚質','本斤量']],
    on='馬名', how='left'
).rename(columns={'本斤量':'today_weight'})
...
def eval_pedigree(row):
    if ped_df is None or row['馬名'] not in ped_df.index:
        return 1.0
...
# 各指標を Z スコア化
metrics = [
    'raw_norm','up3_norm','odds_norm','jin_norm','today_norm',
