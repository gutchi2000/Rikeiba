...
df['Raw'] = df['RawBase'] * df.apply(ped_factor,axis=1) \
                         * df['脚質'].map(style_f) \
                         * df['年齢'].map(age_f) \
                         * df['性別'].map(sex_f) \
                         * df['レース日'].map(sea_f)
...
st.subheader("本日の予想6頭（バランス順）")
top6 = summary.nlargest(6,'バランス').reset_index(drop=True)
st.table(top6[['馬名','偏差値','安定性','バランス']])
...
with col3:
    ...
    st.pyplot(fig1)
with col4:
    st.markdown("**偏差値 vs 安定性 散布図**")
    fig2, ax2 = plt.subplots(figsize=(6,4))
    ax2.scatter(summary['偏差値'], summary['安定性'], alpha=0.7)
    ax2.axvline(summary['偏差値'].mean(), linestyle='--')
    ax2.axhline(summary['安定性'].mean(), linestyle='--')
    for _,r in summary.iterrows():
        ax2.text(r['偏差値'], r['安定性'], r['馬名'], fontsize=6)
