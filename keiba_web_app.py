...
if do_train:
    ...
    if not SKLEARN_AVAILABLE:
        ...
        if len(numeric_cols) == 0:
            ...
            # 2) inf を NaN に置換
            X_train.loc[:, numeric_cols] = X_train.loc[:, numeric_cols].replace([np.inf, -np.inf], np.nan)
            X_val.loc[:, numeric_cols]   = X_val.loc[:, numeric_cols].replace([np.inf, -np.inf], np.nan)
            ...
            med = X_train.loc[:, numeric_cols].median()
            X_train.loc[:, numeric_cols] = X_train.loc[:, numeric_cols].fillna(med)
            X_val.loc[:, numeric_cols]   = X_val.loc[:, numeric_cols].fillna(med)
