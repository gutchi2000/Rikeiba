...
# 日本語フォント設定ont_manager.fontManager.addfont("ipaexg.ttf")
...
if any(c not in df.columns for c in cols):
    st.error(f"不足列: {set(cols)-set(df.columns)}")
    st.stop()

# 前処理
df = df[cols].copy()
for c in cols[2:]: df[c] = pd.to_numeric(df[c], errors='coerce')
...
def eval_pedigree(m):
    if html_file:
        try:
            ...
            ped = tables[0]
            sire = ped.set_index('馬名').get('父馬', {}).get(m, '')
            return 1.2 if sire in ['サクラバクシンオー','スウェプトオーヴァード'] else 1.1 if sire in ['マンハッタンカフェ','フジキセキ'] else 1.0
        except:
            pass
    return 1.0
...
# 脚質評価
def style_factor(m): return {'逃げ':1.2,'先行':1.1,'差し':1.0,'追込':0.9}.get(style_map.get(m,''),1.0)
...
for m in ['pedigree_factor','style_factor','age_factor']: df[f'Z_{m}']=(df[m]-df[m].mean())/df[m].std(ddof=1)

# 合成偏差値化
weights={'Z_raw_norm':8,'Z_up3_norm':2,'Z_odds_norm':1,'Z_jin_norm':1,'Z_wdiff_norm':1,'Z_pedigree_factor':3,'Z_age_factor':2,'Z_style_factor':2}
total=sum(weights.values())
df['total_z']=sum(df[k]*w for k,w in weights.items())/total
z, zmin, zmax = df['total_z'], df['total_z'].min(), df['total_z'].max()
...
sns.barplot(x='バランススコア',y='馬名',data=combined,ax=ax)
st.pyplot(fig)
...
with st.expander('ベット設定'):
    ...
    axis = combined['馬名'].iloc[0] if not combined.empty else t10[0]
    if bt in ['単勝','複勝']:
        r={'単勝':0.25,'複勝':0.75}[bt]
        st.write(f"{bt} {axis} → {(bd*r)//100*100} 円")
    else:
        lst=combined['馬名'].tolist();others=lst[1:]
        combos = ([f"{axis}-{h}" for h in others] if bt in ['馬連','ワイド'] else ["-".join(sorted([lst[0],b,c])) for b in lst[1:3] for c in lst[3:]] if bt=='三連複' else [f"{lst[0]}→{i}→{j}" for i in lst[1:4] for j in lst[1:4] if i!=j])
