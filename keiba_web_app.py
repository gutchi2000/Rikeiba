...
with st.sidebar:
    st.header("重み設定")
    male_w    = st.number_input("牡馬重み", 0.0, 2.0, 1.1)
    female_w  = st.number_input("牝馬重み", 0.0, 2.0, 1.0)
    gelding_w = st.number_input("せん馬重み", 0.0, 2.0, 0.95)
    nige_w   = st.number_input("逃げ重み", 0.0, 2.0, 1.2)
    senko_w  = st.number_input("先行重み", 0.0, 2.0, 1.1)
    sashi_w  = st.number_input("差し重み", 0.0, 2.0, 1.0)
    ooka_w   = st.number_input("追込重み", 0.0, 2.0, 0.9)
    spring_w = st.number_input("春重み", 0.0, 2.0, 1.0)
    summer_w = st.number_input("夏重み", 0.0, 2.0, 1.1)
    autumn_w = st.number_input("秋重み", 0.0, 2.0, 1.0)
    winter_w = st.number_input("冬重み", 0.0, 2.0, 0.95)
    w_best   = st.number_input("ベストタイム重み", 0.0, 1.0, 1.0)
    gate_w   = {i: st.number_input(f"{i}枠重み", 0.0, 2.0, 1.0) for i in range(1,9)}
...
# --- データ読み込み ---
upload = st.file_uploader("XLSX ファイル", type="xlsx")
if not upload:
    st.stop()
...
).fillna(stats["ベストタイム"].str.extract(r"(\d+)")[0].astype(float).max())

df = df.merge(stats[["馬名","性別","年齢","best_dist_time","枠"]], on="馬名", how="left")
...
summary["偏差値"] = 30 + (summary["mean_z"]-summary["mean_z"].min())/(summary["mean_z"].max()-summary["mean_z"].min())*40
summary["実績偏差値"] = 30 + (summary["RawBase_mean"]-summary["RawBase_mean"].min())/(summary["RawBase_mean"].max()-summary["RawBase_mean"].min())*40
summary["バランス"]   = weight_z*summary["偏差値"] + weight_rb*summary["実績偏差値"] - summary["std_z"]

# 出力
st.subheader("本日の予想6頭")
out = summary.nlargest(6,"バランス").reset_index(drop=True)
out["印"] = ["◎","〇","▲","△","△","△"]
