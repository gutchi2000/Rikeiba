...
if upload_html:
    try:
        ...
        ped = pd.read_html(str(soup.find('table')))[0].set_index('馬名')
    except:
        ped = None
...
def ped_factor(r):
    ...
    mother_sire = ped.at[r['馬名'], '母父']
    for term in user_terms:
        if term == r['馬名']:
            score += 2
        if term == father or term == mother_sire:
            score += 1
    return 1.0 + 0.1 * score
...
# 合成スコア total_z
comp_w = {'Z_raw_n':8,'Z_up3_n':2,'Z_odds_n':1,'Z_jin_n':w_jin,'Z_today_n':w_jin,'Z_dist_n':w_best,'Z_wd_n':1}
