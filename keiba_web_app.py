...
with st.sidebar:
    ...
    weight_pedigree = st.slider('血統補正(Z_ped) の重み',0.0, 1.0, 0.1, 0.05)
...
def sea_f(dt):
    m = dt.month
    return (
        spring_w if m in [3,4,5] else
        summer_w if m in [6,7,8] else
        autumn_w if m in [9,10,11] else
        winter_w
    )
...
summary['バランス'] = (
    ...
    weight_pedigree * summary['Z_ped_f'] -
    summary['std_z']
)
...
st.table(top6[['印','馬名','偏差値','RawBase偏差値','Z_ped_f','バランス']])
