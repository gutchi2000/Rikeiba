...
def calc_score(row):
    ...
    jin = row['jinryo']
    jin_mean = df['jinryo'].mean()
    jin_norm = 1 - abs(jin - jin_mean) / jin_mean if jin_mean>0 else 0

    wdiff = row['weight_diff']
    wt_mean = df['weight'].mean()
    wdiff_norm = 1 - abs(wdiff) / wt_mean if wt_mean>0 else 0

    # 重み: raw_norm 8, up3_norm 2, odds_norm 1, jin_norm 1, wdiff_norm 1 => 合計13
    s = raw_norm*8 + up3_norm*2 + odds_norm + jin_norm + wdiff_norm
    return s / 13 * 100
...
x0, y0 = df_avg['偏差値'].mean(), df_avg['標準偏差'].mean()
