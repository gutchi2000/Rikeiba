...
edited = st.data_editor(attrs, column_order=['枠','番','馬名','性別','年齢','脚質'],
...
keys = st.text_area("系統名を1行ずつ入力",height=100).splitlines(); bp=st.slider("血統ボーナス点数",0,20,5)
...
points=alt.Chart(df_agg).mark_circle(size=120).encode(
    x=alt.X('RankZ:Q',title='偏差値',scale=alt.Scale(domain=[df_agg['RankZ'].min()-5,df_agg['RankZ'].max()+5])),
    y=alt.Y('Stability:Q',title='安定度',scale=alt.Scale(domain=[df_agg['Stability'].min()-1,df_agg['Stability'].max()+1])),
    color=alt.condition(
        alt.datum.RankZ>=50 & alt.datum.Stability>=avg_st,
        alt.value('red'), alt.value('steelblue')
    ),
    tooltip=['馬名','AvgZ','Stdev']
)
vline=alt.Chart(pd.DataFrame({'x':[50]})).mark_rule(strokeDash=[5,5]).encode(x='x:Q')
hline=alt.Chart(pd.DataFrame({'y':[avg_st]})).mark_rule(strokeDash=[5,5]).encode(y='y:Q')
labels=alt.Chart(df_agg).mark_text(dx=7,dy=-7,fontSize=11).encode(x='RankZ:Q',y='Stability:Q',text='馬名:N')
chart=(points+vline+hline+labels).properties(width=600,height=400)
