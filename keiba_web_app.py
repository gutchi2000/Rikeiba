...
cand=df_out.copy();cand['comp']=cand['mean_z']-cand['std_z'];cand=cand.merge(df_avg,on='馬名')
...
with st.expander('券種別買い目候補と予算配分'):
    horses=top6['馬名'].tolist();axis=horses[0];others=horses[1:]
    umaren=[f"{axis}-{h}"for h in others];wide=umaren.copy()
    f,s,t=horses[0],horses[1:3],horses[3:]
    sr=["-".join(sorted([f,b,c]))for b in s for c in t]
    fix=horses[:4]
    stn=[f"{fix[0]}→{i}→{j}"for i in fix[1:]for j in fix[1:]if i!=j]
    bt=st.radio('馬連 or ワイド',['馬連','ワイド'])
    ts=['単勝','複勝',bt,'三連複','三連単']
    tb=st.number_input('総ベット予算（円）',min_value=1000,step=1000,value=10000)
    b=tb//len(ts);alloc={t:(b//100)*100 for t in ts[:-1]};alloc[ts[-1]]=((tb-sum(alloc.values()))//100)*100
    rows=[]
    for t,am in alloc.items():
        if t=='単勝':rows.append({'券種':'単勝','組合せ':axis,'金額':int((am*1/4)//100*100)})
        elif t=='複勝':rows.append({'券種':'複勝','組合せ':axis,'金額':int((am*3/4)//100*100)})
