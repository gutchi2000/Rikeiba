...
with st.sidebar:
    ...
    with st.expander("最終スコア重み", expanded=True):
        weight_z    = st.slider('総合偏差値の重み',       0.0, 1.0, 0.7, 0.05)
        weight_rb   = st.slider('実績偏差値の重み',       0.0, 1.0, 0.2, 0.05)
        weight_gate = st.slider('枠順偏差値の重み',      0.0, 1.0, 0.1, 0.05)

# 注意
st.write("**注意：設定変更後は『…』→『Clear cache』で再実行してください。**")
...
def split_frame(x):
    p=str(x).split('-',2)
    return pd.Series({
        '枠': int(p[0]) if p[0].isdigit() else 1,
...
stats['best_dist_time']=pd.to_numeric(
    ...
    stats['ベストタイム'].str.extract(r'(\d+)',expand=False).astype(float).max()
)
# マージ
...
horses=df['馬名'].unique()
input_df=pd.DataFrame({'馬名':horses, '脚質':['差し']*len(horses),'本斤量':[56]*len(horses)})
ed=st.data_editor(
    input_df,
    column_config={
...
def gate_f(r): return gate_weights.get(r['枠'],1)
...
df['Raw']= df['RawBase'] * df['脚質'].map(style_f) * df['年齢'].map(age_f) * df['性別'].map(sex_f) * df['レース日'].map(sea_f) * df.apply(gate_f,axis=1)

# 正規化＆Z化
scores=['Raw','Ave-3F','単勝オッズ','best_dist_time','斤量','today_weight','増減']
for c in scores:
    df[f'Z_{c}']=(df[c]-df[c].mean())/df[c].std(ddof=1)
# 枠順Z
mu_g,sd_g=df['枠'].map(lambda x:gate_weights.get(x,1)).mean(), df['枠'].map(lambda x:gate_weights.get(x,1)).std(ddof=1)
df['Z_枠']=(df['枠'].map(lambda x:gate_weights.get(x,1))-mu_g)/sd_g

# 合成スコア
tmp={'Z_Raw':8,'Z_Ave-3F':2,'Z_単勝オッズ':1,'Z_best_dist_time':w_best,'Z_斤量':w_jin,'Z_today_weight':w_jin,'Z_増減':1,'Z_枠':weight_gate}
