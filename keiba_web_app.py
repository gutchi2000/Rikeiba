...
with st.sidebar:
    ...
    with st.expander("最終スコア重み", expanded=True):
        ...
        weight_gate = st.slider('枠偏差値の重み',      0.0, 1.0, 0.1, 0.05)
...
def seasonal_f(dt):    m=dt.month; return spring_w if m in range(3,6) else summer_w if m in range(6,9) else autumn_w if m in range(9,12) else winter_w
...
df['Raw']     = df['RawBase'] \
                ...
                * df['年齢'].map(age_f) \
                * df['性別'].map(sex_f) \
                * df['レース日'].map(seasonal_f) \
                * df.apply(gate_f, axis=1)
...
# 枠順偏差値（Z_枠）
mu_g = df['枠'].map(lambda x:gate_weights.get(x,1)).mean()
sd_g = df['枠'].map(lambda x:gate_weights.get(x,1)).std(ddof=1)
...
weights_comp = {
    ...
    'Z_斤量': w_jin,
    'Z_today_weight': w_jin,
    'Z_増減': 1,
    'Z_枠': weight_gate
}
df['total_z'] = sum(df[k] * w for k,w in weights_comp.items()) / sum(weights_comp.values())
